...
class BookingRepository
{
    ...
    public static function getBookingDetailinCurrentDate($roomId){
        ...
        'user.firstName', 'user.lastName','user.Department'])->join('user','user.userId','=', 'booking.userId')
    ...
    public static function countbooking($bookingId){
        $today = Carbon::today();
        ...
        ->select(
            'bookings.room_id',
            DB::raw('DATE_FORMAT(bookings.booking_date, "%d/%m/%y") AS datenow'),
            DB::raw('COUNT(*) AS ReservationCount')
        )
        ->groupBy('bookings.room_id', DB::raw('DATE_FORMAT(bookings.booking_date, "%d/%m/%y")'))
        ->orderBy('bookings.booking_time_start', 'asc')
        ->get();  
        return $countbooking;
    }
